...
import { translations, popularStocks, nativeLang, nativeName } from '@/data/translations/myanmar';
...
export default function TradeUSStocksFromMyanmar() {
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <section className="relative py-20 px-4 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-red-500/20 via-gray-950 to-green-500/20" />
        ...
        <div className="absolute top-4 right-4 z-10">
          <LanguageToggle currentLang={lang} nativeLang={nativeLang} nativeName={nativeName} onToggle={(l) => setLang(l as 'en' | 'my')} />
        </div>
        ...
        <div className="max-w-4xl mx-auto relative">
          ...
          <div className="flex items-center gap-2 mb-6">
            <span className="px-3 py-1 bg-yellow-500/20 text-yellow-400 text-sm font-medium rounded-full">{t.badge}</span>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6 leading-tight">
            {t.heroTitle1}
            <br />
            <span className="text-yellow-400">{t.heroTitle2}</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-4 mb-12">
            <a
              href="#get-started"
              className="px-8 py-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-gray-900 rounded-xl font-bold text-lg hover:from-yellow-400 hover:to-yellow-500 transition-all shadow-lg shadow-yellow-500/25"
            >
              {t.startTrading}
            </a>
            <a
      ...
      {/* Why This Works for Myanmar Traders */}
      ...
      <section id="get-started" className="py-16 px-4 bg-gray-900/50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full mb-4">
              <span className="text-yellow-400 font-medium">{t.stepByStep}</span>
            </span>
                ...
                <div className="px-6 pb-6 border-t border-gray-800 pt-6">
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
                      <h4 className="text-blue-400 font-semibold mb-2">{t.waveMoney.title}</h4>
                      <p className="text-gray-400 text-sm mb-3">{t.waveMoney.desc}</p>
                      <ol className="text-xs text-gray-300 space-y-1">
                        {t.waveMoney.steps.map((step, i) => (
                          <li key={i}>{i + 1}. {step}</li>
                        ))}
                      </ol>
                    </div>
                    ...
                    <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
                      <h4 className="text-green-400 font-semibold mb-2">{t.kbzPay.title}</h4>
                      <p className="text-gray-400 text-sm mb-3">{t.kbzPay.desc}</p>
                      <ol className="text-xs text-gray-300 space-y-1">
                        {t.kbzPay.steps.map((step, i) => (
                          <li key={i}>{i + 1}. {step}</li>
                        ))}
                      </ol>
                    </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {popularStocks.map(stock => (
              <Link
                ...
                className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-yellow-500/50 transition-colors"
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white font-bold">{stock.ticker}</span>
                  {stock.hot && <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full">{t.hot}</span>}
                </div>
          ...
          <div className="text-center mt-6">
            <Link href="/stocks" className="text-yellow-400 hover:text-yellow-300 font-medium">
              {t.viewAllStocks}
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 px-4 bg-gray-900/50">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Myanmar Guides Section */}
          <div className="mt-12 bg-gradient-to-r from-yellow-500/10 to-green-500/10 border border-yellow-500/20 rounded-2xl p-6">
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-gradient-to-br from-yellow-500/20 to-green-500/20 border border-yellow-500/30 rounded-2xl p-8 text-center">
            <h2 className="text-3xl font-bold text-white mb-4">
              {t.ctaTitle}
            </h2>
            ...
            <button
              onClick={() => hyperliquid?.referralUrl && trackHyperliquidClick(hyperliquid.referralUrl)}
              className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-gray-900 rounded-xl font-bold text-lg hover:from-yellow-400 hover:to-yellow-500 transition-all shadow-lg shadow-yellow-500/25 cursor-pointer"
            >
              {t.getStartedCta}
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {t.relatedGuides.map((guide, i) => (
              <Link key={i} href={guide.href} className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-yellow-500/50 transition-colors">
                <span className={`text-xs font-medium ${
                  i === 0 ? 'text-yellow-400' : i === 1 ? 'text-green-400' : 'text-blue-400'
                }`}>{guide.tag}</span>
